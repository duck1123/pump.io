...
ActivityObject.isDomainToSkip = function(hostname) {
    ...
    if (_.contains(examples, hostname.toLowerCase())) {
        return true;
    }

    if (_.contains(namespaces, hostname.toLowerCase())) {
        return true;
    }

    parts = hostname.split(".");

    if (_.contains(tlds, parts[parts.length - 1])) {
        return true;
    }
...
ActivityObject.discover = function(obj, callback) {
    ...
    if (!_.contains(["http", "https", "acct"], proto) ||
